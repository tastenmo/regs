...
struct is_register_rw
    : std::integral_constant<bool,
                             (mask == std::numeric_limits<reg_type>::max()) &&
                                 (offset == unsigned{0})> {};

template <typename reg_type, reg_type mask, unsigned offset, typename U>
using is_register =
    typename std::enable_if<is_register_rw<reg_type, mask, offset>::value,
                            U>::type;
...
struct Write {
  ...
  template <typename U = void>
  static void write(
      reg_type &target, reg_type value,
      is_register<reg_type, mask, offset, U> * = nullptr) noexcept {
    target = value;
  }
};
...
struct WriteConstant {
  ...
  template <typename U = void>
  static void write(reg_type &target, is_register<reg_type, mask, offset, U> * =
                                          nullptr) noexcept {
    target = value;
  }
};
...
struct read_write : read_only {
  ...
  static void clear(reg_type &target) noexcept {
    detail::WriteConstant<reg_type, mask, unsigned{0}, (reg_type)~mask>::write(
        target);
  }
...
struct write_only {
  ...
  static void write(const reg_type &target, const reg_type value) noexcept {
    detail::Write<reg_type, type_mask<T>::value, unsigned{0}>::write(
        target, ((value << offset) & mask));
  }

  template <typename reg_type, reg_type mask, unsigned offset, reg_type value>
  static bool write(const reg_type &target) noexcept {
    detail::WriteConstant<reg_type, type_mask<T>::value, offset,
                          ((value << offset) & mask)>::write(target);
  }
};
